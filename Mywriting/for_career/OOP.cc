...
虚基指针这个概念是为了解决菱形继承而引入的
   ...
   B     C
    \   /
     \ /
      D
例如A中有一个数据data，B,C继承A,然后D又继承B,C，此时其实D中还有两个两份A的成员data，访问的时候需要指定是访问
那个对象中的data，D.B::data,D.C::data，这个是比较不符合一般逻辑的。
...
因为 A 现在是 B 和 C 的虚基类，编译器不能再像普通继承那样，在编译时就确定 A 子对象相对于 B 或 C 对象的固定偏移量。
A 的位置可能会根据最终的派生类（如 D）的布局而变化，虚基指针就是为了解决这个动态查找地址的问题而产生的。
+-------------------+
|      D 对象       |
+-------------------+
| B 的部分          |
|  - vbptr (指向 B 的虚基表) |  <-- B 的虚基指针
+-------------------+
| C 的部分          |
|  - vbptr (指向 C 的虚基表) |  <-- C 的虚基指针
+-------------------+
|   D 自己的成员
+-------------------+
| A 的部分 (唯一实例) |
...
vbptr 指向一个名为 ** 虚基表（vbtable）** 的静态数组。
虚基表中存放的不是函数地址，而是偏移量（offsets）。
对于 B 的 vbtable，它会记录从 B 的 vbptr 开始，到唯一的 A 子对象的偏移量。
对于 C 的 vbtable，它也会记录从 C 的 vbptr 开始，到同一个 A 子对象的偏移量
所以访问D中的data变量时，其实是通过选择一个路径来查找到A的，比如通过B的虚基指针定位到
虚基表，然后从虚基表中查询到唯一A子对象的偏移量，进而访问到A对象

注意：虚继承的时候这四个类不仅不会共享虚基类指针，也不会共享虚函数指针
