...
为什么vector是选择每次二倍进行扩容，而不是选择1.5倍或者固定大小进行扩容？
二倍扩容的核心作用是将单次扩容的 O (n) 成本，通过降低扩容频率，均摊到多次插入操作中，最终使每次插入的
均摊时间复杂度为 O (1)。,同时也是平衡空间浪费和扩容频率，
如果扩容倍数太小，则扩容会比较频繁，总的拷贝成本会高，O(n*n),如果扩容倍数太大，空间浪费严重，
两倍扩容能保证有一半的空间被利用。
