...
1.收到可写事件后，采用循环写入的方式进行写事件，同时需要采用非阻塞的套接字，内核缓冲区
...
所以0.1+0.2不能直接==0.3，而是应该两者做差，然后判断差值如果能够小于一定的阈值时就能判断两者是
相等的
...
/*
模板和内联函数是怎么样影响编译的：
编写一个模板函数时，就像是一个蓝图，此时不生成任何的代码，而是当编译器去实际看到代码中实际使用到的实际版本
的时候，才会去创建出模板的实例。并且代码生成是一个耗时的过程，如果模板大量的使用，就会产生大量的实例化，并且可能
会产生大量重复的代码。并且为了能够使得模板在任意位置都能进行实例化，完整的模板定义应该要在头文件中，所以
增加了模板和头文件之间的依赖。内联函数原理相同

模板函数和内联函数之所以深刻影响编译，是因为它们将本可以在链接期处理的工作（函数调用）强行提前到了编译期。
模板要求编译器在编译期 “生成” 新代码，而内联要求编译器在编译期 “展开” 已有代码，这两种操作都会增加编译的计算量、
内存消耗和时间，并可能导致最终代码体积的膨胀。

*/

/*
客户但调用close关闭和调用shutdown关闭的区别：
调用close关闭是同时关闭写方向和读方向，对于多进程或者多线程共享socket的情况下是将引用技术-1，但是需要注意的是
所谓的关闭写方向和读方向并不是一蹴而就的，而是有个过程的，这个过程是：
客户端:   ====>FIN报文(客户端关闭写方向)        服务端
客户端:    (ACK)<===================          服务端
客户端:    (FIN)<==(只有客户端收到服务端发送的FIN报文之后，客户端的读方向才关闭)          服务端
                   所以在读方向关闭之前，服务端还有未发送的数据时，可以先发送数据，然后再发送FIN报文，这就是四次挥手
                  的原因，当服务端没有数据要发送时并且开启TCP延迟确认机制，就可以变成三次挥手

*/

/*
前置声明的好处:
如果说不需要使用到类的成员变量或者说函数或者说也不需要知道类的大小，则可以不需要引入类的头文件，因为头文件包含大量的代码
文件，可以通过前置声明即可。通过前置声明可以定义出该类的指针或者是引用，但是不能定义出类的实例对象，这是因为定义
一个类的实例对象实际上是在栈上开辟一个该对象大小的空间来存储这个对象，所以是涉及到该类的大小了
...
/*
编译的强制依赖：
如果一个文件的编译依赖于其他文件，那其他文件的修改都会引起该文件重新编译。
1.直接代码依赖：
头文件：一个文件需要包含很多头文件，这就是强制依赖，一旦头文件里的内容变化，则必须要重新编译
模板：模板是 “按需编译” 的。编译器需要在使用模板的地方看到完整的模板定义（包括实现），
才能为特定的类型（如 std::vector<int>）生成具体的代码。
内联函数：为了能够在每个调用点进行内联替换，编译器必须在每个调用点都看到函数的完整定义。
因此，内联函数的定义通常也放在头文件中，这就创建了对其定义的强制依赖。

*/

/*
placement new 允许你在已有的内存缓冲区上构造对象，这个内存缓冲区可以是栈上的空间、全局变量空间，
甚至是通过其他方式预先分配好的内存。placement new 需要包含 <new> 头文件，
语法形式为 new (location) Type(arguments) ，其中 location 是指向已分配内存的指针，
Type 是要构造的对象类型，arguments 是构造函数的参数,不过需要手动调用析构函数。

char buffer[sizeof(MyClass)];  // 在栈上创建一个缓冲区
    MyClass* ptr = new (buffer) MyClass;  // 使用 placement new 在 buffer 上构造对象
...
/*
sizeof和strlen的区别:
sizeof:属于运算符，编译期计算，无运行开销
计算的是整个对象 / 类型的内存大小，包括所有占用的字节（如数组的全部元素、字符串的 '\0' 终止符）。
对于数组，sizeof 结果 = 数组长度 × 元素类型大小。
对于字符串字面量或字符数组，sizeof 包含末尾的 '\0'。

strlen:属于库函数，运行时计算
从字符串首地址开始遍历，直到遇到 '\0' 为止，统计之前的字符数（不包含 '\0'）。
依赖字符串必须以 '\0' 结尾，否则会遍历到非法内存，导致未定义行为。

*/

/*
编译时是如何处理左值引用和右值引用的：
类型检查：严格验证引用与左值 / 右值的绑定规则，确保合法性。
引用折叠：简化复杂引用类型，支持万能引用等特性。
语义分析：在重载决议和移动语义中区分引用类型，确保正确行为。
代码生成：将引用实现为常量指针，同时通过优化减少开销
在代码生成阶段，编译器通常将左值引用和右值引用处理为常量指针（即指向对象的地址，且指针本身不可修改），
因此引用的底层内存占用和指针相同（32 位系统 4 字节，64 位系统 8 字节）。
